...
public interface BookService {
	...
	// PK 1개를 전달받아 조회하는 method는 0or1개의 데이터만 return한다
	// 데이터 없을 때는 null을 있을때는 실제 데이터가 담긴 DTO가 return 된다
	public BookDTO findById(String bk_isbn);
	
	// 도서명을 1개 전달받아서 도서명으로 조회를 하고 결과를 return 하는 기능을 수행
	public List<BookDTO> findByTitle(String bk_title);
	
	// 변경할 도서 데이터를 1개를 전달 받아서 데이터를 갱신(변경, 수정) 하는 기능을 수행 
